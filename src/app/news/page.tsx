...
export default async function NewsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const posts = getPublishedPosts(kind);
  ...
  return <InteriorLayout locale={locale} messages={t} user={user} sectionTitle="Forums" sectionItems={forumSectionItems} activeHref={kind ? `/news?kind=${kind}` : undefined} pageTitle={pageTitle}>
      ...
      <section className="article-list">{posts.length ? posts.map((post) => <article key={post.id}><span>{post.kind === "update" ? "UPDATE" : "NOTICE"}</span><div><h2><Link href={`/news/${post.id}`}>{post.title}</Link></h2><p>{post.summary}</p><small>By {post.author_name}</small></div><time>{date.format(new Date(`${post.created_at}Z`))}</time></article>) : <p className="empty-state">No articles have been published yet.</p>}</section>
  </InteriorLayout>;
}
