...
export default async function ArticlePage({ params }: { params: Params }) {
  ...
  return <InteriorLayout locale={locale} messages={t} user={user} sectionTitle="Forums" sectionItems={forumSectionItems} activeHref={activeHref} pageTitle={post.kind === "update" ? "UPDATE NEWS" : "LATEST NEWS"}><article className="full-article"><nav className="breadcrumbs"><Link href="/news">News & Updates</Link><span>›</span><strong>{post.title}</strong></nav><header><span>{post.kind === "update" ? "UPDATE NEWS" : "LATEST NEWS"}</span><h1>{post.title}</h1><p>{post.summary}</p><small>Published by {post.author_name} · {date.format(new Date(post.created_at))}</small></header><div className="article-body">{post.body.split(/\n{2,}/).map((paragraph, index) => <p key={`${post.id}-${index}`}>{paragraph}</p>)}</div></article></InteriorLayout>;
}
